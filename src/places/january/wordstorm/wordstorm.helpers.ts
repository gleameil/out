...
import { play } from '../../../shared/instrument/instrument';
...
function middaySun(parent: HTMLDivElement) {
    for (let i = 0; i < 720; i++) {
        ...
        if (i % 3 === 0 && ray.classList.contains('yellow-ray')) {
            ...
            ray.style.zIndex = '-1';
            ray.addEventListener('mouseover', () => {
              play(i / 3, SUN_RAY_INSTRUMENT)
            });
        }
        ray.style.opacity = '0';
...
export function whiteoutMiddayOrDispensation(words: string[]): FullPoem {
  ...
  function moveSun({ rightBy, upBy, lastPosition = SUN_RAY_START }) {
    ...
    const newPosition: SunRayPosition = { 
      ...
      right: { 
        gold: lastPosition.right.gold - rightBy,
        long: lastPosition.right.long - rightBy,
        white: lastPosition.right.white - rightBy,
        yellow: lastPosition.right.yellow - rightBy,
      }
    }
    for (let i = 0; i < rays.length; i++) {
        let ray = rays[i] as HTMLDivElement;
        ray.style.top = `${newPosition.top}vw`;
        if (ray.classList.contains('long-ray')) {
            ray.style.right = `${newPosition.right.long}vw`;
        } else if (ray.classList.contains('yellow-ray')) {
            ray.style.right = `${newPosition.right.yellow}vw`;
        } else if (ray.classList.contains('white-ray')) {
            ray.style.right = `${newPosition.right.white}vw`;
        } else if (rays[i].classList.contains('gold-ray')) {
            ray.style.right = `${newPosition.right.gold}vw`;
        }
    }
}
  ...
  function midday(clicks: number, time: Time): string | undefined {
    ...
    if (clicks > 1) {
      ...
      moveSun({ rightBy: ((100/36) * (clicks - 1)), upBy: y });
    }
    if (clicks === 1) {
      setBackground(time.backgroundColor, time.background);
      middaySun(parent);    
...
export function bellAndScaleForTime(time: Time): Instrument {
  switch (time.name) {
    case 'weeHours':
      return {
        type: 'bell',
        timbre: 'sine',
        tonalWorld: WORLDS.enigmatic,
      };
    case 'dawn':
      return {
        type: 'bell',
        timbre: 'triangle',
        tonalWorld: WORLDS.pentatonicMinor
      };
    case 'sunrise':
      return {
        type: 'bell',
        timbre: 'sawtooth',
        tonalWorld: WORLDS.fourthsAndFifthsSunset,
      };
    case 'morning':
      return {
        type: 'bell',
        timbre: 'square',
        tonalWorld: WORLDS.pentatonicMajor,
      };
    case 'midday':
      return {
        type: 'bell',
        timbre: 'sine',
        tonalWorld: WORLDS.fourthsAndFifths,
      };
    case 'afternoon':
      return {
        type: 'bell',
        timbre: 'square',
        tonalWorld: WORLDS.major,
      };
    case 'sunset':
      return {
        type: 'bell',
    ...
    case 'dusk':
      return {
        type: 'bell',
        timbre: 'triangle',
        tonalWorld: WORLDS.dorian,
      };
    case 'night':
      return {
        type: 'bell',
        timbre: 'sine',
        tonalWorld: WORLDS.wholeTone,
      };
  }
  return {
    type: 'bell',
...
export function locationForClick(e: MouseEvent) {
  const x = e.pageX;
  return Math.floor(50 * x / window.innerWidth);
}
